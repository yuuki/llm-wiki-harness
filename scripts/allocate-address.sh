...
#
# 採番(引数なし / allocate)は環境変数 WIKI_ALLOCATE_OK=1 のときだけ通す。
# wiki-page-write.py がロック内で付ける。エージェントが直接呼ぶ経路は拒否する。
# --peek / --rebuild は lint 用なのでこの制限の外。
#
# Exit codes:
#   0 — success
...
#   4 — allocate without WIKI_ALLOCATE_OK=1 (direct call refused)
...
case "$MODE" in
  ...
  --rebuild)
    ...
    ;;
  allocate|"")
    if [ "${WIKI_ALLOCATE_OK:-}" != "1" ]; then
      echo "ERR: allocate-address.sh の採番は wiki-page-write.py の内側だけ。直接呼ばない(--peek / --rebuild は可)" >&2
      exit 4
    fi
